...
CNTRCT_OWNG_ORG_T$Unit <- with(CNTRCT_OWNG_ORG_T, as.factor(
  ifelse(grepl('GBS', CNTRCT_OWNG_ORG_NM), 'GBS', 
...
master <- left_join(master, POS_STAT_RESN_T[c('STAT_RESN_CD', 'STAT_RESN_DESC')], by = "STAT_RESN_CD")

master <- left_join(master, CNTRCT_OWNG_ORG_T[c("CNTRCT_OWNG_ORG_ID", "CNTRCT_OWNG_ORG_NM", "Unit")], by = 'CNTRCT_OWNG_ORG_ID')

master <- left_join(master, CNTRCT_TYP_T[c('CNTRCT_TYP_ID', 'CNTRCT_TYP_DESC')], by = 'CNTRCT_TYP_ID')

master <- left_join(master, INDSTR_T, by =  "INDSTR_ID")

security <- left_join(CNTRY_SEC_CLRNCE_T, SEC_CLRNCE_TYP_T, by = "SEC_CLRNCE_TYP_ID")
security <- security[c('CNTRY_SEC_CLRNCE_ID', 'SEC_CLRNCE_TYP_DESC')]
...
master <- tbl_df(master) %>%
  mutate(STAT_RESN_DESC.og = STAT_RESN_DESC,
         STAT_RESN_DESC = replace(STAT_RESN_DESC, 
                                  STAT_RESN_CD %in% c('WB', 'WE', 'WG') & PREF_FULFLMNT_CHNL_CD == 'SUBC',
...
candidate.type <- left_join(OPNSET_POS_CAND_STAT_T, CAND_STAT_T[1:2], by = c('CAND_OPNSET_STAT_ID' = 'CAND_STAT_ID'))

candidate <- left_join(candidate, candidate.type, by = 'OPNSET_POS_CAND_ID')

candidate <- left_join(candidate, CAND_T[c('CAND_ID', 'cand.ctry', 'CAND_SRC_CD')], by = 'CAND_ID')
...
actualizer <- tbl_df(master) %>%
  ...
  mutate(STAT_RESN_DESC = ifelse(is.na(STAT_RESN_DESC), F, STAT_RESN_DESC == 'Staffed by contractor/other'), 
...
actualizer <- merge(test.old, actualizer, 'OPNSET_POS_ID', all.x = T)
#now there's 2 paths:
...
vars <- c("OPNSET_ID", "OPNSET_POS_ID", "STAT_RESN_DESC", "INDSTR_NM", "STRT_DT", "END_DT", "CRE_T", 
          ...
          "FULFILL_RISK_ID", "SEC_CLRNCE_TYP_DESC", "OWNG_CNTRY_CD", "OPPOR_OWNR_NOTES_ID", "WRK_CNTRY_CD", 
...
facts <- c("OPNSET_POS_ID", "STAT_RESN_DESC", "INDSTR_NM", "CNTRCT_OWNG_ORG_NM", "OPPOR_OWNR_NOTES_ID", "Unit",
           "PAY_TRVL_IND", "WRK_RMT_IND", "CNTRCT_TYP_DESC", "FULFILL_RISK_ID", "BND_LOW", "BND_HIGH",
           "SEC_CLRNCE_TYP_DESC", "OWNG_CNTRY_CD", "WRK_CNTRY_CD", "JOB_ROL_TYP_DESC", "SKLST_TYP_DESC", 
           "SET_TYP_CD", "WRK_CTY_NM", 'URG_PRIRTY_IND')
data[facts] <- lapply(data[facts], function(x) as.factor(str_trim(x)))
rm(facts)
...
data$PREF_FULFLMNT_CHNL_CD <- with(data, PREF_FULFLMNT_CHNL_CD == 'SUBC')

#need subk = TRUE
data$NEED_SUB_IND <- with(data, NEED_SUB_IND == 'Y')

#contractor position = TRUE; this is target
data$STAT_RESN_DESC <- with(data, ifelse(is.na(STAT_RESN_DESC), F, STAT_RESN_DESC == 'Staffed by contractor/other'))
...
month.vars <- c("STRT_DT", "END_DT", "CRE_T", "LST_UPDT_T", 'OPN_T', "WTHDRW_CLOS_T")

data[month.vars] <- lapply(data[month.vars], function(x) ymd(str_sub(x, 1, 10)))
...
data$needed30.days <- from.today(data$STRT_DT) <= 30
...
##################NEED TO MOVE THIS TO THE RIGHT SPOT
...
candidate <- left_join(candidate, data[c('OPNSET_ID', 'OG.Start.floor')], by = 'OPNSET_ID')
...
candidate.count <- left_join(candidate.count, candidate.type, by = c('OPNSET_ID', 'OG.Start.floor'))
candidate.count <- candidate.count %>%
  mutate(active.cands = (candidate.count - Confirmed - Withdrawn - `<NA>`)/candidate.count)

#map back into the data
data <- left_join(data, candidate.count, by = c('OPNSET_ID', 'OG.Start.floor'))
...
library(cowsay)
sink(paste0('30 day randomForest diagnostics ', current.run, '.txt'))
...
reports <- c(paste0('30 day - IBM Output Report ', current.run, '.xlsx'),
             paste0('30 day - Vendor-US Output Report ', current.run, '.xlsx'),
             paste0('30 day - Vendor-CA Output Report ', current.run, '.xlsx'))
...
#output IBM report
setwd("~/Demand Forecasting II/Archive")
saveWorkbook(wb, file = paste0('30 day - IBM Output Report ', current.run, '.xlsx'))
setwd("~/Demand Forecasting II/Output Reports")
saveWorkbook(wb, file = paste0('30 day - IBM Output Report.xlsx'))
...
setwd("~/Demand Forecasting II/Archive")
saveWorkbook(wb, file = paste0('30 day - Vendor-US Output Report ', current.run, '.xlsx'))
setwd("~/Demand Forecasting II/Output Reports")
saveWorkbook(wb, file = paste0('30 day - Vendor-US Output Report.xlsx'))
...
setwd("~/Demand Forecasting II/Archive")
saveWorkbook(wb, file = paste0('30 day - Vendor-CA Output Report ', current.run, '.xlsx'))
setwd("~/Demand Forecasting II/Output Reports")
saveWorkbook(wb, file = paste0('30 day - Vendor-CA Output Report.xlsx'))
